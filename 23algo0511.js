...
function kMostFrequent(nums, k) {
    ...
    let newArr = Object.keys(map).sort((a, b) => map[b] - map[a]) //.keys(map) will get all keys withint the map. Chaining .sort will make newArr with keys order by largest to lowest key value
    return newArr.slice(0, k).map(result => parseInt(result)); // 
}
